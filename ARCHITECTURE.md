...
Explicit Transcribe → SQLite job queue → bounded Node worker → AudioTranscriptionProvider
                                      ↓
                         TranscriptIngestionService → SQLite final segments
```
...
Filesystem names use server UUIDs rather than display names. A temporary `0600` file is synced and atomically renamed beneath a `0700` root/session directory. Lexical containment, UUID path grammar, `lstat`, `realpath`, and symlink checks reject traversal/escape. Upload compensates file creation if metadata persistence fails. Before Next.js buffers multipart data, the collection route requires a finite positive decimal `Content-Length` no larger than the configured file limit plus an explicit 64 KiB multipart allowance. Unknown-length/chunked uploads are intentionally rejected in v0.5; the post-parse file-size check remains authoritative for the selected file.

Transcription is separate from upload and GET. POST atomically binds an action receipt to a queued job and returns without provider work. Jobs use only `queued`, `running`, `completed`, `failed`, and `cancelled`; retry waiting is queued with a future `available_at`. A successful guarded claim increments the attempt, assigns a random fixed 120-second lease, and moves the asset to `transcribing`. The worker invokes storage and the provider outside transactions with a 90-second timeout, at most three attempts, retry delays of 1 and 5 seconds, and bounded expired-lease recovery.

`TranscriptIngestionService.ingestUploadedAudio` is the authoritative completion transaction. It validates job/session/asset/action/role ownership plus a matching unexpired lease, inserts every final segment, activates a draft session, completes asset and job, and clears leases/failures atomically. Lease recovery means provider invocation is at least once, while this transaction makes committed transcript effect exactly once. Deletion planning cancels active jobs before marking assets deleting; an already-running provider may continue in memory, but its stale output cannot commit. The only adapter remains deterministic non-production Fake. The embedded worker requires a long-lived local Node process and is disabled by default and always disabled in production.

Filesystem and SQLite deletion are explicitly compensating rather than presented as one atomic resource transaction. `uploaded_audio_deletion_batches` and `uploaded_audio_deletion_files` durably record server-derived original/tombstone paths and progress through `planned → metadata_deleted → completed`. Deletion first transactionally cancels queued or running jobs and marks assets `deleting`, then idempotently renames files to same-directory UUID tombstones. Provider code already running may finish in memory, but lease-guarded completion rejects its stale output. If authoritative asset/session deletion fails, every staged rename is rolled back; failed rollback remains addressable through the durable plan. After authoritative database deletion, unlink and completion marking are retryable, including when the analysis-session row is already gone. Individual completion nulls transcript source links through `ON DELETE SET NULL`; transcript rows remain. An incomplete asset deletion blocks whole-session deletion until its own plan is resumed.
...
## Database maintenance v0.6

Database maintenance is a standalone infrastructure/CLI boundary and is not imported by browser components. Backup uses `better-sqlite3`'s online backup API to snapshot a live WAL database into a destination-directory temporary file. Read-only validation checks the SQLite structure, `quick_check`, foreign keys, user version, and Drizzle migration metadata before a restrictive two-file database/manifest pair is published without overwrite.

Restore remains offline and never runs migrations. It validates the selected pair before inspecting or writing the destination. Replacement validates the current database through a retained, validated pre-restore online backup, copies and validates a candidate beside the destination, moves the original database plus existing WAL/SHM sidecars to rollback names, installs without overwrite, validates the selected bytes and SHA-256, then removes rollback files. Handled failures before final validation restore the original files exactly. `--confirm-offline` is an operator assertion, not process discovery or locking.

The same-directory protocol bounds ordinary filesystem failures but cannot make the multi-file database/WAL/SHM sequence atomic across sudden process termination, host failure, or power loss. A persistent operation lock and retained recovery files make an interrupted attempt fail closed on retry; the operator must inspect recovery files and the retained pre-restore pair before retrying. This is not crash-proof or disaster-recovery certification.

Backup retention is another standalone CLI/infrastructure flow. An isolated strict parser combines CLI-over-environment configuration without loading provider settings. Direct-child discovery recognizes only safe database/manifest names and reuses strict manifest and backup validation. A pure policy sorts valid pairs by manifest `createdAt` with an internal deterministic tie-breaker, protects at least the newest `keepLatest`, and selects only older pairs. Dry-run is the default and does not lock or mutate.

Apply acquires one exclusive owned directory lock before discovery. For each selected pair it opens both files with `O_NOFOLLOW` where supported, retains the handles through mutation, checks discovered/opened/path identity, creates exclusive same-directory recovery hard links, and then removes both normal names. Handled mid-pair failures restore owned normal names without overwriting replacements; failed rollback or cleanup leaves non-normal residue that blocks later discovery. This is a bounded handled-failure protocol, not crash-atomic deletion across sudden termination or power loss.

Operations status is local CLI-only and read-only. Infrastructure pins a bounded regular non-symlink database file, refuses live WAL/SHM sidecars, copies bytes into an in-memory SQLite inspection database, and executes fixed quick-check, foreign-key, migration, transcription-job, and deletion-batch queries. Backup inventory reuses retention discovery. The renderer exposes only the closed snapshot schema and never returns table names, rows, paths, identifiers, or content.

`safe-operational-event.ts` is separate from the existing route logger. Its closed schema accepts fixed maintenance event, operation, outcome, and level values plus bounded error codes, durations, counts, and byte totals. CLI entry points emit one newline-delimited JSON event on stderr after success/degradation/failure; JSON command results remain isolated on stdout. Emission failures cannot alter maintenance semantics, and no telemetry, persistence, shipping, or external request exists.
