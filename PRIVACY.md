...
- **Uploaded Audio:** stores a user-selected authorized practice file beneath the configured server filesystem root and safe metadata in SQLite. Upload does not transcribe. A separate explicit action sends bytes only to the selected server-side transcription adapter. v0.4 includes only a deterministic network-free non-production Fake adapter, so no uploaded audio is sent to OpenAI, DeepSeek, or another external service.
...
Uploaded practice audio is stored beneath `UPLOADED_AUDIO_PATH` (default `./data/uploaded-audio`), never in SQLite or export. The original filename is retained only as bounded display metadata and cannot select the storage path. One declared role applies to the whole file because v0.4 performs no diarization. Transcription text may be inaccurate and is not used for scoring or suggested answers.
